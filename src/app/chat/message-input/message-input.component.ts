...
export class MessageInputComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit {
   ...
   */
  @Output() readonly messageUpdate: EventEmitter<void> = new EventEmitter<void>()
  ...
  typingStart$: Subject<void> = new Subject<void>()
  cooldown$: Observable<number> | undefined
  isCooldownInProgress: boolean = false
  ...
  private readonly defaultTextareaPlaceholder: string = 'streamChat.Type your message'
  private readonly slowModeTextareaPlaceholder: string = 'streamChat.Slow Mode ON'
  ...
  constructor(
    ...
    this.subscriptions.push(
      this.attachmentService.attachmentUploadInProgressCounter$.subscribe((counter) => {
    ...
    this.subscriptions.push(
      this.typingStart$.subscribe(() => void this.channelService.typingStarted(this.parentMessageId))
    )

    this.subscriptions.push(
      combineLatest([this.channelService.latestMessageDateByUserByChannels$, this.channelService.activeChannel$])
        .pipe(
          map(([latestMessages, channel]): [Date | undefined, Channel<DefaultStreamChatGenerics> | undefined] => [
            latestMessages[channel?.cid || ''],
            channel!
          ])
        )
        .subscribe(([latestMessageDate, channel]) => {
          const cooldown =
            ...
            latestMessageDate &&
            Math.round(
              (channel?.data?.cooldown as number) - (new Date().getTime() - latestMessageDate.getTime()) / 1000
            )
          if (cooldown && cooldown > 0 && (channel?.data?.own_capabilities as string[]).includes('slow-mode')) {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.push(
      this.customTemplatesService.attachmentPreviewListTemplate$.subscribe((template) => {
  ...
  async messageSent() {
    ...
    if (attachmentUploadInProgressCounter > 0) {
      if (!this.hideNotification) {
        this.hideNotification = this.notificationService.addPermanentNotification(
  ...
  private initTextarea() {
    ...
    this.textareaRef = this.textareaAnchor.viewContainerRef.createComponent<any>(componentFactory)
    this.cdRef.detectChanges()
  }
  ...
  private async areAttachemntsValid(fileList: FileList | null) {
    ...
    Array.from(fileList).forEach((f) => {
      ...
      if (isImageFile(f)) {
        hasBlockedExtension = !!this.appSettings?.image_upload_config?.blocked_file_extensions?.find((ext) =>
          f.name.endsWith(ext)
        )
        hasBlockedMimeType = !!this.appSettings?.image_upload_config?.blocked_mime_types?.find(
          (type) => f.type === type
        )
        hasNotAllowedExtension =
          !!this.appSettings?.image_upload_config?.allowed_file_extensions?.length &&
        ...
        hasBlockedExtension = !!this.appSettings?.file_upload_config?.blocked_file_extensions?.find((ext) =>
          f.name.endsWith(ext)
        )
        hasBlockedMimeType = !!this.appSettings?.file_upload_config?.blocked_mime_types?.find((type) => f.type === type)
        hasNotAllowedExtension =
          !!this.appSettings?.file_upload_config?.allowed_file_extensions?.length &&
